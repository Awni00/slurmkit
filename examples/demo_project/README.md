...
slurmkit notify job --collection notifications/terminal_failed --job-id 991002 --exit-code 1 --dry-run

# Uses global fallback (model_comparison spec has no notifications block)
slurmkit notify job --collection notifications/terminal_completed --job-id 992011 --exit-code 0 --on always --dry-run
```
...
slurmkit notify collection-final --collection notifications/terminal_completed --job-id 992011 --no-refresh --dry-run
```
...
```bash
slurmkit generate experiments/hyperparameter_sweep/slurmkit/job_spec.yaml --into generated/hyperparameter_sweep
slurmkit generate experiments/model_comparison/slurmkit/job_spec.yaml --into generated/model_comparison
```
...
| `slurmkit status` | Demoed | `slurmkit status fixtures/mixed_30` |
...
| `slurmkit submit` | Demoed (`--dry-run` locally) | `slurmkit submit generated/hyperparameter_sweep --dry-run` |
| `slurmkit resubmit` | Demoed (`--dry-run`/fixture collections) | `slurmkit resubmit fixtures/mixed_30 --filter failed --dry-run` |
| `slurmkit collections list` | Demoed | `slurmkit collections list` |
| `slurmkit collections show` | Demoed | `slurmkit collections show fixtures/mixed_30` |
| `slurmkit collections analyze` | Demoed | `slurmkit collections analyze fixtures/mixed_30` |
| `slurmkit collections refresh` | Partially demoed | Works best with real SLURM job IDs |
| `slurmkit collections delete` | Demoed | `slurmkit collections delete generated/hyperparameter_sweep -y` |
| `slurmkit notify test` | Demoed | `slurmkit notify test --dry-run` |
| `slurmkit notify job` | Demoed (dummy context + dry-run) | `slurmkit notify job --collection notifications/terminal_failed --job-id 991002 --exit-code 1 --dry-run` |
...
slurmkit notify collection-final \
  --collection notifications/terminal_failed \
  --job-id 991002 \
  --no-refresh \
  --dry-run

slurmkit notify collection-final \
  --collection notifications/terminal_completed \
  --job-id 992011 \
  --no-refresh \
  --dry-run

slurmkit notify collection-final \
  --collection notifications/in_progress \
  --job-id 993020 \
  --no-refresh \
  --dry-run
```
...
```bash
slurmkit notify collection-final --collection notifications/terminal_failed --job-id 991002 --no-refresh
slurmkit notify collection-final --collection notifications/terminal_failed --job-id 991002 --no-refresh
slurmkit notify collection-final --collection notifications/terminal_failed --job-id 991002 --no-refresh --force
```
...
```bash
export PYTHONPATH="$PWD:$PYTHONPATH"
# Already enabled by spec override for notifications/terminal_failed
slurmkit notify job --collection notifications/terminal_failed --job-id 991002 --exit-code 1 --dry-run

# Also enabled by spec override for notifications/terminal_failed
slurmkit notify collection-final --collection notifications/terminal_failed --job-id 991002 --no-refresh --dry-run
...
```bash
slurmkit notify test --route local_email --dry-run
slurmkit notify job --collection notifications/terminal_failed --job-id 991002 --exit-code 1 --route local_email --dry-run
```
...
```bash
slurmkit notify test --route local_email
slurmkit notify job --collection notifications/terminal_failed --job-id 991002 --exit-code 1 --route local_email
```
...
```bash
slurmkit generate experiments/hyperparameter_sweep/slurmkit/job_spec.yaml --into generated/hyperparameter_sweep
slurmkit submit generated/hyperparameter_sweep --delay 2
slurmkit status generated/hyperparameter_sweep
slurmkit collections refresh generated/hyperparameter_sweep
slurmkit collections show generated/hyperparameter_sweep
slurmkit collections analyze generated/hyperparameter_sweep
slurmkit resubmit generated/hyperparameter_sweep --filter failed
```
